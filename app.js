...
const session = require('express-session');            // תוסף לניהול sessions
...
app.use(session({
    ...
    cookie: { secure: false }                          // הגדר ל-true אם משתמשים ב-HTTPS
}));

// ===== Middleware =====
const { setDefaultUser } = require('./middlewares/auth');
app.use(setDefaultUser);

// ===== ראוטים =====
const viewRoutes = require('./routes/viewRoutes');     // מייבא את הנתיבים של התצוגה (דפים שמבוססים על EJS)
const cartRoutes = require('./routes/cart');     // מייבא את הנתיבים של העגלה

app.use('/', viewRoutes);                              // מפעיל את כל הראוטים תחת הנתיב הראשי (כמו '/', '/books', וכו')
app.use('/', cartRoutes);                              // מפעיל את נתיבי העגלה
